...
Kc=  1./chi0;%gamm.*p0./( 1 + (gamm-1).*tanh(sqrt(Pr).*sc.*sqrt(-1i))./(sqrt(Pr).*sc.*sqrt(-1i)) );              % effective modulus of the cavity (slit)
Kt=  1./chi0;%gamm.*p0./( 1 + (gamm-1).*tanh(sqrt(Pr).*st.*sqrt(-1i))./(sqrt(Pr).*st.*sqrt(-1i)) );               % effective modulus of the tube (slit)
Kn=  1./chi0;%gamm.*p0./( 1 + (gamm-1).*tanh(sqrt(Pr).*sn.*sqrt(-1i))./(sqrt(Pr).*sn.*sqrt(-1i)) );              % effective modulus of the neck (slit)
