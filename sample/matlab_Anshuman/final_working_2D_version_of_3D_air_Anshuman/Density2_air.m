...
%checked
Ctr= (1i.*omega.*ht.*chit)./(omega.^2.*rhot.*chit-k.^2);
...
Ccr=(1i.*omega.*(hc)./Kc)./(omega.^2.*rhoc./Kc-k.^2);
...
B1= [-exp(-1i.*kt.*Lx./2).*exp(1i.*k.*Lx), -exp(1i.*kt.*Lx./2).*exp(1i.*k.*Lx), exp(1i.*kt.*Lx./2)...
    exp(-1i.*kt.*Lx./2); 
-exp(-1i.*kt.*Lx./2).*exp(1i.*k.*Lx), exp(1i.*kt.*Lx./2).*exp(1i.*k.*Lx), exp(1i.*kt.*Lx./2), -exp(-1i.*kt.*Lx./2);
1, 1, -1, -1;
-1, -1, 0, 0;
1, -1, -1, 1;
];
...
-(Ycn/Yct).*exp(-1i.*kn.*hn./2), (Ycn/Yct).*exp(1i.*kn.*hn./2), 0, 0, 0, 0;
exp(1i.*kn.*hn./2), exp(-1i.*kn.*hn./2) -1, -1, 0, 0;
(Ycn/Ycc).*exp(1i.*kn.*hn./2), -(Ycn/Ycc).*exp(-1i.*kn.*hn./2), 1, -1, -1, 1];

B5= [0, 0, 1, 1, -1, -1;
0, 0, 1.*exp((-1./2).*1i.*kc.*(wc)), -1.*exp((1./2).*1i.*kc.*(wc)), 0, 0;
0, 0, 0, 0, 1.*exp((1./2).*1i.*kc.*(wc)), -1.*exp((-1./2).*1i.*kc.*(wc))];



M= [[B1; B3] [B2; B4; B5]];                                              % Constructing the coef matrix AA by its blocks
Crho=[0; 0; 0; Btr; 0; Bcr; 0; 0; -Ccr.*exp((-1./2).*1i.*k.*(wc))./Ycc; -Ccr.*exp((1./2).*1i.*k.*(wc))./Ycc];      % vector of constants: AA * X = CC

Cchi=[0; 0; 0; Btc-Bnc; 0; Bcc-Bnc; 0; 0; -Ccc.*exp((-1./2).*1i.*k.*(wc))./Ycc;...
    -Ccc.*exp((1./2).*1i.*k.*(wc))./Ycc];
 Cchi;
...
average_pvr=average_pvr +Btr.*Ctr.*(exp(1i.*k.*Lx)-1.)./(2.*1i.*k)...
    ...
    +Ar(3).*Ctr.*(exp(1i.*(k+kt).*Lx./2)-1.)./(1i.*(k+kt))...
...
average_vc= Ctc.* ( exp(1i.*k.*Lx./2)-exp(-1i.*k.*Lx./2) ) ./1i./k...
    ...
    + Yct.*Ac(3).*(exp(1i.*kt.*Lx./2)-1.)./1i./kt...
    -Yct.*Ac(4).*(exp(-1i.*kt.*Lx./2)-1.)./(-1i)./kt +Ycc.*Ac(9).*(exp(1i.*kc.*(wc)./2)-1.)./1i./kc...
    -Ycc.*Ac(10).*(exp(-1i.*kc.*(wc)./2)-1.)./(-1i)./kc;  
average_vc=average_vc./(Lx*Ly);
...
average_pvc=average_pvc +Btc.*Ctc.*(exp(1i.*k.*Lx)-1.)./(2.*1i.*k) ...
    +Btc.*Yct.*Ac(3).*(exp(1i.*(k+kt).*Lx./2)-1.)./(1i.*(k+kt)) ...
    -Btc.*Yct.*Ac(4).*(exp(1i.*(k-kt).*Lx./2)-1.)./(1i.*(k-kt)) ...
    +Ac(3).*Ctc.*(exp(1i.*(k+kt).*Lx./2)-1.)./(1i.*(k+kt))...
    +Yct.*Ac(3).^2.*(exp(1i.*kt.*Lx)-1.)./(2.*1i.*kt)...
    + Ac(4).*Ctc.*(exp(1i.*(k-kt).*Lx./2)-1.)./(1i.*(k-kt))...
    -Yct.*Ac(4).^2.*(exp(-1i.*kt.*Lx)-1.)./(-2.*1i.*kt);


average_pvc= average_pvc+Bcc.*Ccc.*(exp(1i.*k.*(wc))-1.)./(2.*1i.*k)...
    +Bcc.*Ycc.*Ac(9).*(exp(1i.*(k+kc).*(wc)./2)-1.)./(1i.*(k+kc))...
    -Bcc.*Ycc.*Ac(10).*(exp(1i.*(k-kc).*(wc)./2)-1.)./(1i.*(k-kc))...
    +Ac(9).*Ccc.*(exp(1i.*(k+kc).*(wc)./2)-1.)./(1i.*(k+kc))...
    +Ycc.*Ac(9).^2.*(exp(1i.*kc.*(wc))-1.)./(2.*1i.*kc) ...
    + Ac(10).*Ccc.*(exp(1i.*(k-kc).*(wc)./2)-1.)./(1i.*(k-kc))...
    -Ycc.*Ac(10).^2.*(exp(-1i.*kc.*(wc))-1.)./(-2.*1i.*kc);

average_pvc=average_pvc./(Lx*Ly);
...
average_bc=2.*1i.*Ctc.*sin(kt.*Lx./2) -Yct.*Ac(1).*exp(-1i.*kt.*Lx./2)...
    +Yct.*Ac(2).*exp(1i.*kt.*Lx./2) +Yct.*Ac(3).*exp(1i.*kt.*Lx./2) -Yct.*Ac(4).*exp(-1i.*kt.*Lx./2);
average_bc=average_bc./(1i.*omega.*(Lx*Ly));
