...
load rhoLocal.txt
load chiLocal.txt
load deltavLocal.txt 
klpi=rhoLocal(:,1);
Omega = klpi*c0*pi/L;
cL=1./sqrt((rhoLocal(:,2)+1i.*rhoLocal(:,3)).*(chiLocal(:,2)+1i.* ...
                                               chiLocal(:,3)));
kL=Omega./cL; 
Rec = real(cL);
Imc = imag(cL); 
...
cfig = figure();
        ...
        if(savefig==1); saveas(cfig, strcat(savefigname,extension)); end; 
        if(saveeps==1); saveas(cfig,strcat(savefigname,'.eps'),'epsc'); ...
        end; 

        cfig = figure();
        semilogx(deltavLocal(:,1),deltavLocal(:,2),'--r','LineWidth',2);
        set(gca,'XLim',xlimit1,'XTick',xtick1);
        xlabel(xlab);
        ylabel('viscous boundary layer [m]');
        savefigname='Local_deltav';
        if(savefig==1); saveas(cfig, strcat(savefigname,extension)); end; 
        if(saveeps==1); saveas(cfig,strcat(savefigname,'.eps'),'epsc'); ...
        end; 
